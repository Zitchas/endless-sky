...
WeaponInfoPanel::WeaponInfoPanel(PlayerInfo &player, int index)
...
void WeaponInfoPanel::Draw()
{
  ...
  //  Rectangle cargoBounds = interface->GetBox("cargo");
   ...
   // DrawOutfits(interface->GetBox("outfits"), cargoBounds);
	DrawWeapons(interface->GetBox("new weapons"));
...
bool WeaponInfoPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	else if(key == 's')
	{
		...
		{
			GetUI()->Pop(this);
			GetUI()->Push(new ShipInfoPanel(player, shipIndex));
		}
	}
	...
	else if(canEdit && key == 'D')
	{
		if(shipIt->get() != player.Flagship())
			GetUI()->Push(new Dialog(this, &WeaponInfoPanel::Disown, "Are you sure you want to disown \""
				+ shipIt->get()->Name()
...
void WeaponInfoPanel::DrawWeapons(const Rectangle &bounds)
{
	...
	for(const Hardpoint &hardpoint : ship.Weapons())
	{
		...
		int type = hardpoint.IsPylon() ? 2 : hardpoint.IsTurret() ? 1 : 0;
		++count[isRight][type];
	...
	static const double LABEL_DX = 405.;
	...
	double height = 20. * (gunRows + turretRows + pylonRows) + 10. * (gunRows && turretRows);
	
	double gunY = bounds.Top() + .5 * (bounds.Height() - height);
	double turretY = gunY + 20. * gunRows + 30. * (gunRows != 0);
	double pylonY = turretY + 20. * turretRows + 30. * (turretRows != 0);
	double nextY[2][3] = {
	...
	const double labelCenter[2] = {-.5 * LABEL_WIDTH - LABEL_DX + 70, LABEL_DX + .5 * LABEL_WIDTH - 70};
	...
	for(const Hardpoint &hardpoint : ship.Weapons())
	{
		string name = "[empty]";
		string range = "[0]";
		string defensive = hardpoint.IsDefensive() ? "[Def]" : hardpoint.IsAntiMissile() ? "[AM]" : "[Off]";
		if(hardpoint.GetOutfit())
		{
			name = font.Truncate(hardpoint.GetOutfit()->Name(), 200);
		  //  int displayMass = hardpoint.GetOutfit()->Mass();
			int displayRange = hardpoint.GetOutfit()->Range();
		...
		double x = centerX + (isRight ? LABEL_DX +20 : (-LABEL_DX - font.Width(name) - 20));
		double xx = centerX + (isRight ? LABEL_DX - 75 : (-LABEL_DX - font.Width(range) + 75));
		double xxx = centerX + (isRight ? LABEL_DX - 25 : (-LABEL_DX - font.Width(defensive) + 25));
		bool isHover = (index == hoverIndex);
		font.Draw(name, Point(x, y + TEXT_OFF), isHover ? bright : dim);
		font.Draw(range, Point(xx, y + TEXT_OFF), isHover ? bright : dim);
		font.Draw(defensive, Point(xxx, y + TEXT_OFF), isHover ? bright : dim);
		...
		//float under = (hardpoint.IsUnder() ? .5f : .25f);
		Color color(high, .75f * high, .25f, 1.f);
		if(isTurret)
			color = Color(.25f, .75f * high, high, 1.f);
		if(isPylon)
			color = Color(.25f, high, .65f * high, 1.f);
		
		// Draw the line.
		Point from(fromX[isRight] * 0.8, zoneCenter.Y());
...
void WeaponInfoPanel::DrawCargo(const Rectangle &bounds)
{
	...
	// Cargo list.
	const CargoHold &cargo = ((player.Cargo().Used() && !parkedCargo && ship.GetSystem() == player.GetSystem()) ? player.Cargo() : ship.Cargo());
	Table table;
	table.AddColumn(0, Table::LEFT);
	table.AddColumn(WIDTH - 20, Table::RIGHT);
