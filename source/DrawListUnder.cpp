...
// Clear the list.
void DrawListUnder::Clear(int step, double zoom)
{
	items.clear();
	this->step = step;
	this->zoom = zoom;
...
void DrawListUnder::SetCenter(const Point &center, const Point &centerVelocity)
{
	this->center = center;
...
bool DrawListUnder::Add(const Body &body, double cloak)
{
...
bool DrawListUnder::Add(const Body &body, Point position)
{
	position -= center;
	Point blur = body.Velocity() - centerVelocity;
...
bool DrawListUnder::AddUnblurred(const Body &body)
{
	Point position = body.Position() - center;
	Point blur;
	if(Cull(body, position, blur))
...
bool DrawListUnder::AddSwizzled(const Body &body, int swizzle)
{
	Point position = body.Position() - center;
	Point blur = body.Velocity() - centerVelocity;
	if(Cull(body, position, blur))
...
bool DrawListUnder::Cull(const Body &body, const Point &position, const Point &blur) const
{
...
void DrawListUnder::Push(const Body &body, Point pos, Point blur, double cloak, double clip, int swizzle)
{
